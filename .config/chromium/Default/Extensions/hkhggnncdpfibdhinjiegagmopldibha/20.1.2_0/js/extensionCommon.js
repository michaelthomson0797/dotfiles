...
if (location.href.indexOf("popup.html") != -1) {
	inPopup = true;
} else if (location.href.indexOf("reminders.html") != -1) {
	inReminders = true;
}
...
if (inPopup) {
	...
	polymerFile = "vulcanized-polymer-reminders.html";
} else {
...
var polymerPromise = new Promise(function(resolve, reject) {
	...
	}, inPopup && DetectClient.isChrome() ? 1 : 150);
});
...
function showLoading() {
	// if there's an error toast don't display loading because it will kill error toast
	if (!$("#error")[0].opened) {
...
function patchToRemoveStyleScope(target) {
	var observer = new MutationObserver(function(mutations) {
		mutations.forEach(function(mutation) {
			$(mutation.target).removeClass("style-scope");
			if (mutation.addedNodes) {
				$(mutation.addedNodes).removeClass("style-scope");
			}
		});    
	});
	var config = { attributes: true, subtree: true, childList:true, attributeFilter:["class"] };
