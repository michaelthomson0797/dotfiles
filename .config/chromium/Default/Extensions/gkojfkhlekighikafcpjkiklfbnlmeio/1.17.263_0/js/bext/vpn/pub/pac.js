...
E.load_pac_file = function(last, force){
    ...
    E.has_pac = false;
    if (!chrome || version_util.cmp(be_util.version(), '1.1.798')<0)
	return;
...
E.set_proxy_for_url = function(url, proxy_str){
    ...
    if (chrome && +user_agent.guess_browser().version>=52)
    {
    ...
    xhr.open('POST', 'http://local.hola/?'+zescape.qs({
        ...
        set: url,
        key: E.pac_key||'1',
    }, {space_plus: false}), zutil.is_mocha());
    E.stats.total++;
    t0 = Date.now();
