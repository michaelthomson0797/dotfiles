    ...
    function(be_config, version_util, be_util, $)
{
    ...
    if (version_util.cmp(be_util.version(), '1.2.800')<0)
...
E.init = function(){
	...
	if (!E.$el)
	{
	    ...
	    window.ui_popup = E;
	    if (version_util.cmp(be_util.version(), '1.1.864')<0)
		...
		/* XXX arik BACKWARD: <1.2.29 */
	        if (version_util.cmp(be_util.version(), '1.2.29')>=0)
	...
	{
	    if (!B.bg.RMT.ver ||
		version_util.cmp(B.bg.RMT.ver, '1.1.550')<0)
	    {
	    ...
	    if (B.bg.RMT.ver &&
		version_util.cmp(B.bg.RMT.ver, '1.1.583')<0)
	    {
	...
	var backward = (chrome && !conf.new_ui) ||
	    version_util.cmp(be_util.version(), '1.1.812')<0;
	if (!backward)
...
E.load_new_ver = function(ver){
	...
	if (window.hola && window.hola.no_be_ver)
	{
		...
		if (version_util.cmp(be_util.version(), '1.2.166')<=0)
		    country = '';
...
function active_tab_fixup(){
    if (window.is_tpopup || window.top.be_test_page)
        return;
...
function check_ver(){
	...
	if (version_util.cmp(E.R.get('ver'), '1.1.880')<0)
	{
