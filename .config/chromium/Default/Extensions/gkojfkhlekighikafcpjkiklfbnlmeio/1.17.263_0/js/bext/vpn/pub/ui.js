...
define(['jquery', 'underscore', '/bext/pub/backbone.js', '/util/etask.js',
    ...
    '/bext/vpn/pub/defines.js', '/util/util.js',
    ...
    be_defines, zutil, zdot, share, cross_sale, install_exe,
...
function unsupported_open(type, domain, country){
    ...
    be_popup_lib.perr_ok({id: 'be_ui_vpn_click_unsupported_open_'+type, info: {
        src: 'ext',
        browser: browser,
        root_url: domain,
...
var country_list_view_class = Backbone.View.extend({
    ...
    render_list: function(opt){
        ...
        _.forEach(p, function(p){
            list_html += get_country_list_item(countries.get(p.proxy_country),
                false);
        });
        list_html += '<li class="divider"></li>';
        countries.forEach(function(c){
            list_html += get_country_list_item(c, false); });
        this.$ul.on('click', '.country', function(){
            var info = $(this).data();
            if (!info.disable)
...
function must_install_exe(){
    ...
    return browser=='firefox' && (be_util.os_win() || be_util.os_mac()) &&
        !E.be_mode.get('svc.detected') && version_util.cmp(
        storage.get('install_version'), '1.11.965')>0;
}
